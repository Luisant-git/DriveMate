...
export const customerLoginOtp = async (req, res) => {
  ...
  const logData = {
    ...
    templateName: 'customer_login_otp1'
  };
...
export const leadBookingAssignment = async (req, res) => {
  const startTime = Date.now();
  const logData = {
    timestamp: new Date().toISOString(),
    phone: req.body.phone,
  ...
  try {
    ...
    console.log(`[WhatsApp] Starting lead assignment template send:`, logData);
    ...
    // Template message payload (5 parameters matching driver_booking_assignment1 structure)
    const messagePayload = {
      messaging_product: 'whatsapp',
      to: formattedPhone,
      type: 'template',
    ...
    console.log(`[WhatsApp] Sending lead assignment to Meta API:`, {
      url: whatsappConfig.apiUrl,
      to: formattedPhone,
      template: templateName
    });
    ...
    console.error(`[WhatsApp] LEAD ASSIGNMENT ERROR:`, errorLog);
    
    res.status(500).json({ 
      success: false, 
      error: 'Failed to send WhatsApp lead assignment template',
      details: error.response?.data?.error?.message || error.message
    });
  }
};

export const leadBookingConfirmation = async (req, res) => {
  const startTime = Date.now();
  const logData = {
    timestamp: new Date().toISOString(),
    phone: req.body.phone,
  ...
  try {
    ...
    console.log(`[WhatsApp] Starting lead confirmation template send:`, logData);
    ...
    // Template message payload (6 parameters matching driver_booking_confirmation2 structure)
    const messagePayload = {
      messaging_product: 'whatsapp',
      to: formattedPhone,
      type: 'template',
    ...
    console.log(`[WhatsApp] Sending lead confirmation to Meta API:`, {
      url: whatsappConfig.apiUrl,
      to: formattedPhone,
      template: templateName
    });
    ...
    console.error(`[WhatsApp] LEAD CONFIRMATION ERROR:`, errorLog);
