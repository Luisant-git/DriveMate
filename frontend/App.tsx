...
const App: React.FC = () => {
  ...
  const handleLogout = async () => {
    try {
      ...
      setLoginStep('SELECT_ROLE'); // Reset login step for next login attempt
    }
  };
          ...
          >
            ...
            <div className="w-full max-w-md relative z-10">
              ...
              <div className="bg-white rounded-2xl p-5 sm:p-6 shadow-2xl relative overflow-hidden min-h-[400px] flex flex-col">
                ...
                {loginStep === 'SELECT_ROLE' && (
                  <div className="animate-fade-in flex-grow flex flex-col justify-center">
                    ...
                    <div className="space-y-3 sm:space-y-4">
                      <button onClick={() => handleRoleSelect(UserRole.CUSTOMER)} className="w-full flex items-center justify-between p-3 sm:p-4 bg-gray-100 hover:bg-gray-200 rounded-xl transition-colors group">
                        <div className="flex items-center gap-3 sm:gap-4">
                          ...
                          <div className="text-left"><span className="block font-bold text-base sm:text-lg">Customer</span><span className="text-xs text-gray-500">Book a ride</span></div>
                        </div>
                      ...
                      <button onClick={() => handleRoleSelect(UserRole.DRIVER)} className="w-full flex items-center justify-between p-3 sm:p-4 bg-gray-100 hover:bg-gray-200 rounded-xl transition-colors group">
                        <div className="flex items-center gap-3 sm:gap-4">
                          <div className="w-10 h-10 bg-black text-white rounded-full flex items-center justify-center shrink-0">
                            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" /></svg>
                          </div>
                          <div className="text-left"><span className="block font-bold text-base sm:text-lg">Driver</span><span className="text-xs text-gray-500">Drive & Earn</span></div>
                        </div>
                        <svg className="w-5 h-5 text-gray-400 group-hover:text-black transform group-hover:translate-x-1 transition" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
                      </button>
                      <button onClick={() => handleRoleSelect(UserRole.LEAD)} className="w-full flex items-center justify-between p-3 sm:p-4 bg-gray-100 hover:bg-gray-200 rounded-xl transition-colors group">
                        <div className="flex items-center gap-3 sm:gap-4">
                          <div className="w-10 h-10 bg-black text-white rounded-full flex items-center justify-center shrink-0">
                            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" /></svg>
                          </div>
                          <div className="text-left"><span className="block font-bold text-base sm:text-lg">Lead</span><span className="text-xs text-gray-500">Lead Driver</span></div>
                        </div>
                        <svg className="w-5 h-5 text-gray-400 group-hover:text-black transform group-hover:translate-x-1 transition" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
                      </button>
                    </div>
                ...
                {loginStep === 'CUSTOMER_LOGIN' && <CustomerLogin onLogin={handleLogin} onBack={handleBack} />}
                {loginStep === 'DRIVER_LOGIN' && <DriverLogin onLogin={handleLogin} onBack={handleBack} />}
                {loginStep === 'LEAD_LOGIN' && <LeadLogin onLogin={handleLogin} onBack={handleBack} onRegister={() => setLoginStep('LEAD_REGISTER')} />}
                {loginStep === 'LEAD_REGISTER' && <LeadRegister onBack={() => setLoginStep('LEAD_LOGIN')} />}
                {loginStep === 'ADMIN_LOGIN' && <AdminLogin onLogin={handleLogin} onBack={handleBack} />}
              </div>
            </div>
            <Toast />
          </div>
