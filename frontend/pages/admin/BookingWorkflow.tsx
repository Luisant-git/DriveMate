          ...
          {responses.length === 0 ? (
            ...
            <div className="space-y-3">
              {responses.map(response => {
                ...
                return (
                  <div key={response.id} className={`bg-white rounded-xl shadow-sm border-2 ${isAccepted ? 'border-green-200' : response.status === 'REJECTED' ? 'border-red-200' : 'border-gray-200'}`}>
                    <div className="p-5">
                      <div className="flex items-center gap-3 mb-4">
                        <div className={`w-12 h-12 rounded-full flex items-center justify-center font-semibold text-lg flex-shrink-0 ${isAccepted ? 'bg-green-600 text-white' : response.status === 'REJECTED' ? 'bg-red-600 text-white' : 'bg-gray-900 text-white'}`}>
                          {person?.name?.[0] || 'D'}
                        </div>
